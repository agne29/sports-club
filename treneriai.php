...
if (isset($_POST['registruoti'])) {
    ...
    $telefono_numeris = mysqli_real_escape_string($con, $_POST['telefono_numeris']);
    $elektroninis_pastas = mysqli_real_escape_string($con, $_POST['elektroninis_pastas']);
    $tabelio_nr = mysqli_real_escape_string($con, $_POST['tabelio_nr']);

    if (!$error) {
        if (mysqli_query($con, "INSERT INTO treneriai(asmens_kodas, vardas, pavarde,telefono_numeris,elektroninis_pastas,tabelio_nr)
            VALUES('" . $asmens_kodas . "', '" . $vardas . "', '" . $pavarde . "','" . $telefono_numeris . "', '" . $elektroninis_pastas . "', '" . $tabelio_nr . "')")) {
            $successmsg = "Sėkmingai užregistravote trenerį!";
        } else {
    ...
    <div class="row">
        <div class="col-md-4 col-md-offset-4 well">
            <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="patalposforma">
                <fieldset>
                    <legend>Trenerių registracija</legend>
